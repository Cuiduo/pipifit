...
<template>
    <div slot="adult">
         <swiper  class="swipersymptom" dots-position="center"  dots-class="dotsClass">
            <swiper-item v-for="(list,index) in listdata" :key="index" @click="test(index)">
                <ul>
                    <li v-for="(item, indexs) in list" :key="indexs" :class="{active:item.relatedSymptom==0}" @click="symptoms2(indexs,index,item)">{{item.symptomName}}</li>
                </ul>
            </swiper-item>
            <!-- <swiper-item >
                <ul>
...
export default {
  ...
  methods:{
    ...
    //   if (this.waitindex = index) {
    //     this.num = this.waitnum;
    //   } else {
    //     this.num = -1;
    //   }
    // },
    getmsg(){
      ...
      this.symptomData = this.get_indexData[1].crowdList[this.agequjian].symptomList;
